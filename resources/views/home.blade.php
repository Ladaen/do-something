        ...
        <div class="col-sm-4 mb-3">
          <form action="{{ route('activity.store') }}" method="POST">
            ...
            <div class="card h-100">
              <div class="card-header">{{ $NewActivity['type'] }}</div>
                <div class="card-body">
                  <h5 class="card-title">{{ $NewActivity['activity'] }}</h5>
                  <p class="card-text">
                  ...
                  <input type="hidden" name="status" value="unfinisshed">
                  <input type="hidden" name="activity_id" value="{{ $NewActivity['key'] }}">
                  <button type="submit" class="btn btn-primary align-self-end" onClick="this.form.submit(); this.disabled=true; this.innerText='Adding…';">(+) Add to my list</button>
                </div>
              </div>
          </form>
        </div>       
        ...
        <div class="col-sm-4 mt-3">
            <div class="card border-primary">
              <div class="card-body text-primary">
                <h5 class="card-title">Not found any interesting for you?</h5>
                <p class="card-text">Don't Worry, lets generate some other</p>
                <a href="." class="btn btn-primary">Generate more fun</a>
              </div>
            </div>
          </div>     
      </div>
